...
export default function Contacto() {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col max-w-md mx-auto p-6 bg-gris-heavy">
      ...
      <input type="text" id="user_nombre" name="user_nombre" required className="mb-4 p-2 border border-gray-300 rounded focus:ring-6 focus:ring-doradoSK" />
      ...
      <select id="servicio" name="servicio" required className="mb-4 p-2 border border-gray-300 rounded focus:ring-6 focus:ring-doradoSK">
        ...
        <option value="otro" className="bg-gris-heavy text-white">Otro</option>
      </select>
      <label htmlFor="user_mensaje" className="mb-2 font-semibold">Mensaje:</label>
      <textarea id="user_mensaje" name="user_mensaje" rows={5} required className="mb-4 p-2 border border-gray-300 rounded focus:ring-6 focus:ring-doradoSK"></textarea>
      <button type="submit" className="bg-doradoSK text-black font-bold w-[200px] m-[0_auto] p-2 rounded hover:bg-doradoSK-dark transition-colors">Enviar</button>
    </form>
  )
}
