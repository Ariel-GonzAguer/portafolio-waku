export const proyectos = [
  {
    id: '0',
    nombre: 'Pasaporte Cercevero Festival Ulloa',
    descripcion:
      'Aplicación web que se usó como pasaporte digital para el Festival Cercevero Ulloa.',
    tecnologias: ['React', 'Zustand', 'React-PDF', 'QRCode'],
    enlace: 'https://fest-ulloa.netlify.app/',
    tags: ['aplicación web', 'festival', 'pasaporte', 'Netlify'],
    img: ['/imagenes/proyectos/pasaporte.webp'],
  },
  {
    id: '1',
    nombre: 'Porto Burger',
    descripcion: 'Sitio web para un negocio de hamburguesas veganas, con menú y pedidos en línea.',
    tecnologias: ['React', 'CSS', 'Sonner', 'Zustand'],
    enlace: 'https://portoburger.netlify.app/',
    tags: ['comida', 'compras en línea', 'hamburguesas', 'Netlify'],
    img: ['/imagenes/proyectos/porto.webp'],
  },
  {
    id: '2',
    nombre: 'Shuttle 506 Jaco',
    descripcion: 'Sitio web diseñado para un negocio de transporte privado en Costa Rica.',
    tecnologias: ['Astro', 'EmailJS', 'QRCode', 'Wouter'],
    enlace: 'https://shuttle506jaco.com/',
    tags: ['sitio web', 'SEO', 'transporte', 'Costa Rica'],
    img: ['/imagenes/proyectos/shuttle.webp'],
  },
  {
    id: '3',
    nombre: 'Evelyn Astorga - Lash Artist',
    descripcion:
      'Sitio web para una artista de pestañas, con carrito de compras y reservas en línea.',
    tecnologias: ['React', 'CSS', 'Sonner', 'Zustand'],
    enlace: 'https://evelynastorgalashartist.com/',
    tags: ['belleza', 'servicios', 'pestañas', 'React'],
    img: ['/imagenes/proyectos/evelyn-lash.webp'],
  },
  {
    id: '4',
    nombre: 'SUPER KEG',
    descripcion:
      'Aplicación web para gestión inteligente de barriles para cervecerías artesanales.',
    tecnologias: ['React', 'Zustand', 'MichiRouter', 'Firebase'],
    enlace: 'https://superkeg.beer',
    tags: ['PWA', 'cerveza artesanal', 'gestión administrativa', 'kegs'],
    img: ['/imagenes/proyectos/sk-laptop.webp', '/imagenes/proyectos/sk-movil.webp'],
  },
];
